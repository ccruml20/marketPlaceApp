...
class Cart extends Component {
  ...
  onDelete(_id){
    const currentProjectToDelete = this.props.cart;
    const indexToDelete = currentProjectToDelete.findIndex(
      function(cart){
        return cart._id === _id;
      }
    )
    let cartAfterDelete = [...currentProjectToDelete.slice(0, indexToDelete), ...currentProjectToDelete.slice(indexToDelete + 1)];
      ...
      return(
        <Panel key={cartArr._id}>
          <Row>
            ...
            <Col xs={12} sm={2}>
              <h6>Items: <Label bsStyle="success">{cartArr.quantity}</Label> </h6>
            </Col>
            <Col xs={6} sm={4}>
              <ButtonGroup style={{minWidth: '300px'}}>
                ...
                <span>     </span>
                <Button onClick={this.onDelete.bind(this, cartArr._id)} bsStyle="danger" bsSize="small">DETLETE</Button>
              </ButtonGroup>
            </Col>
          </Row>
        </Panel>
